    ...
    <Box mt={4}>
      <Box display="flex" flexDirection={"column"} mb={2}>
        ...
        <Accordion>
          ...
          >
            <Typography variant="h3">Filter{activeFilters > 0 && <> ({activeFilters+1} active filter{activeFilters > 1 ? "s" : ""})</>}
            </Typography>
          </AccordionSummary>
          <AccordionDetails>
            <Filter onActiveChannelsChange={setActiveChannels} onActiveInputsChange={setActiveInputs} onActiveOutputsChange={setActiveOutputs} onActiveTypesChange={setActiveTypes} />
          </AccordionDetails>
        </Accordion>
      </Box>
      ...
      <Box display={"flex"} flexDirection={"column"} gap={2}>
        ...
        >
          ...
          {messages.filter((msg) => {
            ...
            <Grid container key={index} sx={{ backgroundColor: index % 2 === 0 ? "inherit" : "rgb(24,24,24)" }} p={1}>
              <Grid size={{ xs: 1 }}>
              ...
              <Grid size={{ xs: 2 }}>{typeToLabel(msg.type)} ({msg.type})</Grid>
              ...
              <Grid size={{ xs: 2 }}>
                ...
                {(msg.type === 128 || msg.type === 144) && msg.note && <>, {msg.note}</>}
                {(msg.type === 128 || msg.type === 144) && msg.velocity && <>, {msg.velocity}</>}
                {(msg.type === 176) && msg.controller && <>, {msg.controller}</>}
