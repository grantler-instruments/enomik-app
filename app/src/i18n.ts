  ...
  resources: {
    en: {
      translation: {
        ...
        "home_caption": "Tired of writing the same code over and over? Use the intuitive no-code configurator to assign MIDI messages to pins and vice versa, and set up wireless MIDI connections effortlessly.",
        ...
        "deploy_toast_summary": "Sent {{pinConfigs}} pin configs and {{peerCount}} wireless MIDI addresses.",
        "deploy_toast_summary_reset_ok": "Device reset confirmed. Sent {{pinConfigs}} pin configs and {{peerCount}} wireless MIDI addresses.",
        "deploy_toast_summary_reset_timeout": "Reset confirmation not received (timed out). Sent {{pinConfigs}} pin configs and {{peerCount}} wireless MIDI addresses. If this error persists, please update the client firmware to the latest version.",
        ...
        "pin_mode_analog_desc": "Read a continuous value (e.g. from a potentiometer or sensor).",
        "pin_mode_digital_in_desc": "Digital input. Reads LOW/HIGH, use with switches or buttons (with external resistors).",
        "pin_mode_digital_in_pullup_desc": "Digital input with internal pull-up. Use for buttons wired to ground; idle HIGH, pressed = LOW.",
        "pin_mode_touch_desc": "Capacitive touch input. Use Threshold to tune touch sensitivity.",
        "pin_mode_digital_out_desc": "Digital output. Drives LOW/HIGH for LEDs, relays or other digital loads.",
        "pin_mode_pwm_desc": "PWM output. Use for dimming LEDs or controlling motor speed with an analog-like value.",
        "pitch_bend_precision_info": "min/max values are transmitted as 7 bit values, you might lose precision. the underlying config api needs some adjustments. the actual pitchbend values are sent as 14bit values, no worries.",
        ...
        "firmwareuploader_step1": "Step 1 — Select firmware",
        "firmwareuploader_step2": "Step 2 — Bootloader mode",
        "firmwareuploader_step3": "Step 3 — Flash firmware",
        "firmwareuploader_releases_page": "GitHub releases page",
        "firmwareuploader_manual_bootloader_required": "Manual bootloader required (hold BOOT + RESET)",
        "firmwareuploader_load_failed": "Failed to load firmware.",
        "debugger_init_prompt": "To use the MIDI features, please initialize the MIDI system.",
        ...
        "inspector_info": "The Inspector allows you to select MIDI devices and synchronize settings. Because who knows what one did months ago.",
        "inspector_sync": "sync",
    ...
    de: {
      translation: {
        ...
        "home_caption": "Müde von immer wieder demselben Code schreiben? Verwende den intuitiven No-Code-Konfigurator, um MIDI-Nachrichten Pins zuzuordnen und umgekehrt, und richte mühelos drahtlose MIDI-Verbindungen ein.",
        ...
        "deploy_toast_summary": "{{pinConfigs}} Pin-Konfigurationen und {{peerCount}} drahtlose MIDI-Adressen gesendet.",
        "deploy_toast_summary_reset_ok": "Geräte-Reset bestätigt. {{pinConfigs}} Pin-Konfigurationen und {{peerCount}} drahtlose MIDI-Adressen gesendet.",
        "deploy_toast_summary_reset_timeout": "Keine Reset-Bestätigung (Zeitüberschreitung). {{pinConfigs}} Pin-Konfigurationen und {{peerCount}} drahtlose MIDI-Adressen gesendet. Wenn dieser Fehler weiterhin auftritt, aktualisiere bitte die Client-Firmware auf die neueste Version.",
        ...
        "min": "Min",
        "max": "Max",
        "pin_conflict": "Pin {{pin}} wird sowohl als Eingang als auch als Ausgang verwendet",
        "pin_mode_analog_desc": "Liest einen kontinuierlichen Wert (z. B. von einem Potentiometer oder Sensor).",
        "pin_mode_digital_in_desc": "Digitaler Eingang. Liest LOW/HIGH, für Schalter oder Taster (mit externen Widerständen).",
        "pin_mode_digital_in_pullup_desc": "Digitaler Eingang mit internem Pull-up. Für Taster gegen Masse; im Ruhezustand HIGH, gedrückt = LOW.",
        "pin_mode_touch_desc": "Kapazitiver Touch-Eingang. Verwende den Schwellenwert, um die Berührungsempfindlichkeit einzustellen.",
        "pin_mode_digital_out_desc": "Digitaler Ausgang. Schaltet LOW/HIGH für LEDs, Relais oder andere digitale Lasten.",
        "pin_mode_pwm_desc": "PWM-Ausgang. Zum Dimmen von LEDs oder zur Steuerung der Motordrehzahl mit einem analogähnlichen Wert.",
        "pitch_bend_precision_info": "Min-/Max-Werte werden als 7-Bit-Werte übertragen, du könntest an Genauigkeit verlieren. Die zugrunde liegende Config-API benötigt noch einige Anpassungen. Die tatsächlichen Pitchbend-Werte werden als 14-Bit-Werte gesendet, keine Sorge.",
        "composer_start_byte": "Start-Byte",
        ...
        "firmwareuploader_issue_tracker": "GitHub Issue Tracker",
        "firmwareuploader_step1": "Schritt 1 — Firmware auswählen",
        "firmwareuploader_step2": "Schritt 2 — Bootloader-Modus",
        "firmwareuploader_step3": "Schritt 3 — Firmware flashen",
        "firmwareuploader_releases_page": "GitHub-Releases-Seite",
        "firmwareuploader_manual_bootloader_required": "Manueller Bootloader erforderlich (BOOT + RESET halten)",
        "firmwareuploader_load_failed": "Firmware konnte nicht geladen werden.",
        "debugger_init_prompt": "Um die MIDI-Funktionen zu nutzen, initialisiere bitte das MIDI-System.",
        ...
        "inspector_info": "Der Inspector ermöglicht es dir, MIDI-Geräte auszuwählen und Einstellungen zu synchronisieren. Denn wer weiß schon, was man vor Monaten gemacht hat.",
    ...
    es: {
      translation: {
        ...
        "home_caption": "¿Cansado de escribir el mismo código una y otra vez? Usa el intuitivo configurador sin código para asignar mensajes MIDI a pines y viceversa, y configurar conexiones MIDI inalámbricas sin esfuerzo.",
        ...
        "deploy_toast_summary": "Enviadas {{pinConfigs}} configuraciones de pin y {{peerCount}} direcciones MIDI inalámbricas.",
        "deploy_toast_summary_reset_ok": "Reinicio del dispositivo confirmado. Enviadas {{pinConfigs}} configuraciones de pin y {{peerCount}} direcciones MIDI inalámbricas.",
        "deploy_toast_summary_reset_timeout": "No se recibió confirmación de reinicio (tiempo agotado). Enviadas {{pinConfigs}} configuraciones de pin y {{peerCount}} direcciones MIDI inalámbricas. Si este error persiste, actualiza el firmware del cliente a la última versión.",
        ...
        "pin_mode_analog_desc": "Lee un valor continuo (p. ej. de un potenciómetro o sensor).",
        "pin_mode_digital_in_desc": "Entrada digital. Lee LOW/HIGH, úsalo con interruptores o botones (con resistencias externas).",
        "pin_mode_digital_in_pullup_desc": "Entrada digital con pull-up interno. Para botones conectados a tierra; en reposo HIGH, presionado = LOW.",
        "pin_mode_touch_desc": "Entrada táctil capacitiva. Usa el umbral para ajustar la sensibilidad al tacto.",
        "pin_mode_digital_out_desc": "Salida digital. Genera LOW/HIGH para LEDs, relés u otras cargas digitales.",
        "pin_mode_pwm_desc": "Salida PWM. Úsala para atenuar LEDs o controlar la velocidad de un motor con un valor analógico.",
        "pitch_bend_precision_info": "los valores mín/máx se transmiten como valores de 7 bits, podrías perder precisión. la API de configuración subyacente necesita algunos ajustes. los valores reales de pitchbend se envían como valores de 14 bits, sin preocupaciones.",
        ...
        "firmwareuploader_step1": "Paso 1 — Seleccionar firmware",
        "firmwareuploader_step2": "Paso 2 — Modo bootloader",
        "firmwareuploader_step3": "Paso 3 — Flashear firmware",
        "firmwareuploader_releases_page": "página de versiones de GitHub",
        "firmwareuploader_manual_bootloader_required": "Bootloader manual requerido (mantén BOOT + RESET)",
        "firmwareuploader_load_failed": "No se pudo cargar el firmware.",
        "debugger_init_prompt": "Para usar las funciones MIDI, inicializa el sistema MIDI.",
        ...
        "inspector_info": "El Inspector te permite seleccionar dispositivos MIDI y sincronizar la configuración. Porque quién sabe lo que uno hizo hace meses.",
    ...
    zh: {
      translation: {
        ...
        "home_caption": "厌倦了一遍又一遍地编写相同的代码？使用直观的无代码配置器将 MIDI 消息分配给引脚，反之亦然，并轻松设置无线 MIDI 连接。",
        "home_reliable_wireless_midi": "可靠的无线 MIDI",
        "home_without_bluetooth": "无需蓝牙。",
        "home_fully_bidirectional": "完全双向",
        "home_fully_bidirectional_description": "既可作为 MIDI 输入设备，也可作为 MIDI 输出设备。将你的 ESP32 配置为独立的 MIDI 控制器，或作为可通过 MIDI 控制的乐器或声音装置。",
        ...
        "home_kit_description": "使用我们定制的硬件套件快速上手，包括小巧的无线 MIDI 适配器和多功能客户端板——轻松构建你自己的 MIDI 设备所需的一切。",
        "home_esp_now_midi": "前来救场！它提供快速、稳定、低延迟的无线 MIDI，非常适合现场演出、乐器和嵌入式系统。",
        "home_utilities": "配备实用工具",
        "home_utilities_description": "MIDI 监视器和固件上传器",
        "home_utilities_description_includes": "该应用内置了",
        "home_utilities_description_includes_continues": "用于实时消息跟踪，以及一个",
        ...
        "getting_started_upload_dongle_description": "你可以从 ESP-NOW MIDI Arduino 库上传 example/dongle，或使用本应用中的固件上传器工具来刷写预编译的适配器固件。",
        "getting_started_upload_dongle_description_extra": "如果你不需要无线 MIDI 连接，只想将客户端板用作有线 MIDI 设备，可跳过此步骤。",
        "getting_started_upload_client_title": "将固件上传到客户端板",
        "getting_started_upload_client_description": "现在，让我们对客户端板重复此过程。你可以从 ESP-NOW MIDI Arduino 库上传 example/client，或使用本应用中的固件上传器工具来刷写预编译的客户端固件。",
        "getting_started_configuration_title": "配置",
        "getting_started_configuration_description": "有趣的部分！使用配置器将 MIDI 消息映射到引脚，反之亦然，并在你的设备之间设置无线 MIDI 连接。",
        "getting_started_debugging_title": "调试",
        "getting_started_debugging_description": "当出现意外情况时，使用本应用中的 MIDI 监视器工具查看传入和传出的 MIDI 消息以进行调试。",
        ...
        "configurator_info": "为了配置你的 enomik 客户端设备，你需要先将相应的固件刷写到它们上面。你可以使用便捷的固件上传器工具将预编译的固件上传到基于 ESP32 的客户端板。",
        "deploy_toast_summary": "已发送 {{pinConfigs}} 个引脚配置和 {{peerCount}} 个无线 MIDI 地址。",
        "deploy_toast_summary_reset_ok": "设备重置已确认。已发送 {{pinConfigs}} 个引脚配置和 {{peerCount}} 个无线 MIDI 地址。",
        "deploy_toast_summary_reset_timeout": "未收到重置确认（超时）。已发送 {{pinConfigs}} 个引脚配置和 {{peerCount}} 个无线 MIDI 地址。如果此错误持续存在，请将客户端固件更新到最新版本。",
        "firmwareuploader_intro": "这是一项实验性功能，在 Chrome 浏览器中效果最佳。如遇到任何问题，请向我们报告。",
        "firmwareuploader_releases_link": "找不到你的固件，或需要特定版本？你可以从我们的",
        "firmwareuploader_custom_bin": "或上传自定义 .bin 文件",
        "firmwareuploader_bootloader_mode": "某些开发板（如 LOLIN S2 Mini）必须通过按住 BOOT 并按下 RESET 手动进入引导加载程序模式。这是刷写所必需的，与任何软件设置无关。",
        "firmwareuploader_bootloader_mode_instruction": "将设备置于手动引导加载程序模式：按住 BOOT 并按下 RESET。",
        "firmwareuploader_bootloader_mode_confirmation": "我已将设备置于引导加载程序模式",
        "browser_warning": "此应用在 Chrome 或基于 Chromium 的浏览器中效果最佳。某些功能（例如串行、固件上传器）在你的浏览器中可能不可用。",
        "browser_warning_unsupported_title": "不支持的浏览器",
        "browser_warning_unsupported": "你的浏览器不支持 Web Serial API，而连接设备和上传固件需要该 API。强烈建议使用 Chrome 以获得最佳体验。",
        "browser_warning_brave_title": "检测到 Brave 浏览器",
        "browser_warning_brave": "Brave 的隐私护盾可能会阻止 Web Serial 和 Web MIDI API。如果遇到连接问题，请尝试为此页面禁用护盾。强烈建议使用 Chrome 以获得最佳体验。",
        "browser_warning_chromium_title": "浏览器兼容性",
        "browser_warning_chromium": "大多数功能应该可以在你的浏览器中正常工作，但强烈建议使用 Chrome 以获得最佳体验。",
        "browser_warning_unknown_title": "未知浏览器",
        "browser_warning_unknown": "无法识别你的浏览器。此应用需要 Web Serial API 支持。强烈建议使用 Chrome 以获得最佳体验。",
        ...
        "pin_mode_digital_in_desc": "数字输入。读取 LOW/HIGH，与开关或按钮配合使用（需外接电阻）。",
        "pin_mode_digital_in_pullup_desc": "带内部上拉的数字输入。用于接地的按钮；空闲时为 HIGH，按下时为 LOW。",
        "pin_mode_touch_desc": "电容式触摸输入。使用阈值来调整触摸灵敏度。",
        "pin_mode_digital_out_desc": "数字输出。为 LED、继电器或其他数字负载提供 LOW/HIGH。",
        "pin_mode_pwm_desc": "PWM 输出。用于调暗 LED 或以类模拟值控制电机速度。",
        "pitch_bend_precision_info": "最小/最大值以 7 位值传输，你可能会损失精度。底层配置 API 还需要一些调整。实际的弯音值以 14 位值发送，无需担心。",
        ...
        "firmwareuploader_step1": "步骤 1 — 选择固件",
        "firmwareuploader_step2": "步骤 2 — 引导加载程序模式",
        "firmwareuploader_step3": "步骤 3 — 刷写固件",
        "firmwareuploader_releases_page": "GitHub 发布页面",
        "firmwareuploader_manual_bootloader_required": "需要手动引导加载程序（按住 BOOT + RESET）",
        ...
        "inspector_info": "检查器允许你选择 MIDI 设备并同步设置。因为谁还记得几个月前做了什么呢。",
        "inspector_sync": "同步",
